...
int RangeData::setAngle() {
	...
	if(avalid_short) {
		...
		angled = acos(min(1.0, (double)(((int)temp_range[0]/300 + (int)temp_range[2]/300 + 1)*300)/(data.laserL_a + data.laserL_b + OFFSET))) * 180 / PI;
		alpha = min(1.0, max(0.0, 7.62*((((int)temp_range[0]/300 + (int)temp_range[2]/300 + 1)*300)/(data.laserL_a + data.laserL_b + OFFSET)-0.786))); //30 degree turn range 0-1
		printf("Angle: %f\tAlpha: %f\tValue: %i Distance: %f\n", angled, alpha, ((int)temp_range[0]/300 + (int)temp_range[2]/300 + 1)*300, data.laserL_a + data.laserL_b + OFFSET);
	}
	return 0;
}
