...
int RangeData::setAngle() {
	...
	if(avalid_short) {
		angled = acos(min(1.0, 300.0/(data.laserS_a + data.laserS_b + 2*OFFSET))) * 180 / PI;
		alpha = max(0.0, 7.62*(300.0/(data.laserS_a + data.laserS_b)-0.866)); //30 degree turn range 0-1
