...
int RangeData::getRangeShort() {
    return data.laserS_a + data.laserS_b;
}

int RangeData::getRangeLong() {
    return data.laserL_a + data.laserL_b;
}

//pos or negative
int RangeData::getRangeOffset() {
    if(avalid_short) {
        return data.laserS_a + data.laserS_b + ALPHAOFFSET - 300;
    } else if(avalid_long) {
        return data.laserL_a + data.laserL_b + ALPHAOFFSET - this->numwallslong*300;
    }
}
...
{
    const size_t imuSize = myRobot->imuDataList.size();
