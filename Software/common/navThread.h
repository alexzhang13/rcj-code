...
public:
    ...
    inline void UpdatePositionSLAM() {
        slamOut << myRobot->rangeDataList[myRobot->rangeDataList.size()-1].data.tstamp << " c " << myRobot->rangeDataList[myRobot->rangeDataList.size()-1].coord.x_glob + currTile.x*300 << " " << myRobot->rangeDataList[myRobot->rangeDataList.size()-i].coord.y_glob + currTile.y*300 << " " << myRobot->imuDataList[myRobot->imuDataList.size()-1].m_yaw << endl;
    }
